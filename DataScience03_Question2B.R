...
for (i in 1:1000)        #starts a 500 rounds boucle
{
  ...
  Sigmashrinktemp=DefiniteMatrixIdentity+((1-wshrink)*diag(452))%*%sigmainvtemp
  Sigmashrinkinvtemp=solve(Sigmashrinktemp)
  
  
  temp=1/gamma*Sigmashrinkinvtemp%*%mu            #computes markowitz weights using the sigma of the subsample and the population mean
  wsigma=rbind(wsigma,t(temp))              #the new sigma is the same sigma plus the vector temp
  temp=1/gamma*Sigmashrinkinvtemp%*%mutemp        #computes markowitz using subsample mean and sigma
